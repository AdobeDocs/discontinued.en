...
Cross-Device Analytics used the Private Graph to stitch data together. The Private Graph is a repository of hashed device IDs that is specific to your organization. CDA regularly communicates with the device graph to link devices together.
...
* Your implementation must use the latest version of the Visitor ID Service (`VisitorAPI.js`). See the [Home Page](https://experienceleague.adobe.com/docs/id-service/using/home.html) in the Visitor ID Service user guide. Most implementations using [Tags](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html) in Adobe Experience Platform likely already have the Visitor ID Service deployed.
* Your implementation must call the `setCustomerIDs` function (or SDK equivalent) whenever an individual can be identified, such as when a user logs in or opens an email. This requirement applies to all platforms, including mobile apps if used. See [`setCustomerIDs`](https://experienceleague.adobe.com/docs/id-service/using/id-service-api/methods/setcustomerids.html) in the Visitor ID Service user guide.
